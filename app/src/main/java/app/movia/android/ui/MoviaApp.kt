...
internal fun nextEpisodeTitle(current: String): String? {
    ...
    if (seasonMatch != null) {
        ...
        return nextEpisodeTitleForCounts(current, seasonEpisodeCounts)
    }

    // Legacy episode titles are pure formatting and should not trigger a catalog/network lookup.
    if (Regex("^(.*) · E(\\d{2}) · Эпизод (\\d+)$").matches(current)) {
        return nextEpisodeTitleForCounts(current, emptyList())
    }
