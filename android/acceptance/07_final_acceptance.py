...
def media3_playing_evidence(
    client: HttpClient,
    expected_media_id: str,
    *,
    timeout_seconds: float,
) -> Dict[str, Any]:
    """Require real Media3 playback, not merely a completed control action."""
    samples: List[Dict[str, Any]] = []
    deadline = time.monotonic() + min(15.0, max(4.0, timeout_seconds / 4.0))
    while time.monotonic() < deadline:
        response = client.request("/diagnostics")
        payload = response.get("payload")
        media3 = payload.get("media3") if isinstance(payload, dict) else None
        if response.get("status") == 200 and isinstance(media3, dict):
            sample = {
                "state": text_value(media3.get("playbackState")).upper(),
                "isPlaying": bool(media3.get("isPlaying")),
                "playWhenReady": bool(media3.get("playWhenReady")),
                "mediaItemId": text_value(media3.get("mediaItemId")),
...
def playback_probe(
    ...
    *,
    ...
    terminal_result = operation_result(final_operation)
    expected_media_id = text_value(
        terminal_result.get("mediaId") or arguments.get("mediaId")
    )
    media3_evidence = (
        media3_playing_evidence(
            client,
            expected_media_id,
            timeout_seconds=timeout_seconds,
        )
        if terminal_state == "COMPLETED"
        else {"playable": False, "samples": [], "error": "operation not completed"}
    )
    if terminal_state == "COMPLETED":
        # Keep the device cool after collecting the evidence used by this
        # gate; this does not change the recorded playback result.
        client.request(
            "/action",
            method="POST",
            payload={"action": "player.pause", "arguments": {}},
        )
    ...
    pipeline_ok = terminal_state == "COMPLETED" and bool(media3_evidence.get("playable"))
    if not pipeline_ok:
        pipeline_status = (
            "diagnostics_pipeline_failure"
            if terminal_state == "COMPLETED"
    ...
    emit(
        checks,
        "P-PERF-02",
        "final media.play probe reaches completed operation and real playback",
        pipeline_ok,
        {
            "operationId": operation_id,
            "finalStatus": terminal_state,
            "media3Playable": media3_evidence.get("playable"),
    ...
    emit(
        checks,
        "P-PERF-04",
        "Media3 reaches READY/isPlaying for the requested media and position moves",
        bool(media3_evidence.get("playable")),
        {
            "expectedMediaId": expected_media_id,
            "evidence": media3_evidence,
        },
        required=True,
    )
    emit(
