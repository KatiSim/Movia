...
voices, qualities, and classifies results into PLAYABLE, NO_SOURCE, BROKEN_SOURCE, PLAYER_FAILURE.
"""
...
def run_playback_test(item: Dict[str, Any], season: Optional[int] = None, episode: Optional[int] = None) -> Dict[str, Any]:
    ...
    # Pause player after checking to keep device cool and memory clean
    ...
    if not final_op:
        ...
        err_msg = str(final_op.get("error") or "")
        if "не удалось найти" in err_msg.lower() or "no_playable" in err_msg.lower() or stream_count == 0:
