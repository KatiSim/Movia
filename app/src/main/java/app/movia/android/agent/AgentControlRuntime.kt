    ...
    private fun startMediaOperation(args: JSONObject, requestId: String): JSONObject {
        ...
        operationScope.launch(Dispatchers.IO) {
            ...
            try {
                ...
                val minimalPlaybackContent = (requestedTitle?.takeIf { it.isNotBlank() }
                    ?: mediaId?.takeIf { it.isNotBlank() })?.let { fallbackTitle ->
                    MediaContent(
                        id = mediaId?.takeIf { it.isNotBlank() } ?: fallbackTitle,
                        title = fallbackTitle,
                ...
                withContext(Dispatchers.Main) {
                    ...
                    session?.start(
                        ...
                        sourceUri = exactKnown?.url ?: knownStreams.firstOrNull()?.url,
